...
describe('Tezos transaction', function() {
  describe('should parse', () => {
    it('unsigned transaction', async () => {
      ...
      await tx.fromSerializedTransaction(unsignedSerializedOriginationTransaction);
    ...
    it('signed transaction', async () => {
      const tx = new Transaction(coins.get('txtz'));
      await tx.fromSerializedTransaction(signedSerializedOriginationTransaction);
  ...
  describe('should fail', () => {
    it('to parse and invalid parsed transaction', async () => {
      const tx = new Transaction(coins.get('txtz'));
      await tx.fromParsedTransaction({} as ParsedTransaction).should.be.rejected();
    });
