...
export class Transaction extends BaseTransaction {
   ...
   */
  async fromSerializedTransaction(serializedTransaction: string): Promise<void> {
    this._encodedTransaction = serializedTransaction;
    try {
      const parsedTransaction = await localForger.parse(serializedTransaction);
      await this.fromParsedTransaction(parsedTransaction);
    } catch (e) {
      ...
      await this.fromParsedTransaction(parsedTransaction, transactionId);
    }
  }
   ...
   */
  async fromParsedTransaction(parsedTransaction: ParsedTransaction, transactionId?: string): Promise<void> {
    if (transactionId) {
  ...
  async sign(keyPair: KeyPair): Promise<void> {
    ...
    await this.fromParsedTransaction(this._parsedTransaction, this._id);
